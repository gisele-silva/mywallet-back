...
export async function loginUser(req, res) {
    try {
        ...
        const verificaUser = await db.collection('users').findOne({
            email: user.email,
        })
        ...
        const passwordSemCriptografia = bcrypt.compareSync(user.password, verificaUser.password);

        if (passwordSemCriptografia){
            const token = uuid();
